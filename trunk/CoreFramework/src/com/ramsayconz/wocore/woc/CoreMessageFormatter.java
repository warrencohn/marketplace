...
public class CoreMessageFormatter extends WOComponent {
	...
	public String formattedString() {
		...
		return (length > 0 ? StringUtils.left(MessageFormat.format(formatter, args), length) : 
	...
	private String _formatter() {
		...
		if (null == formatString) {
			...
			if (null == noValueString) {
				logger.error("*** value/novalue bindings are both null !");
			...
			if (!(formatString instanceof String)) {
				logger.warn("value binding is a " + formatString.getClass().getName() + ".");
				formatString = formatString.toString();
			}
		}

		logger.trace("FORMAT: " + (String) formatString);
			...
			for (String bindingName : _source().bindingKeys()) {		// Enumerate the list
				if (bindingName.startsWith(_SUB_KEY)) { // Ensure it begins with the expected value
					...
					if (index != _DEFAULT_INDEX) { // If the index is palatable
						...
						// Get the binding from the associations
						Object binding = _source().valueForBinding(bindingName);
						if (binding == null) {
							binding = _EMPTY_STRING;
						}

						// Insert it into the correct location
						_insertBindingIntoArrayAtIndex(binding, arguments, index);
					}
				}
			}
		}
	...
	private WOComponent _source() {
		if (this._source == null) { // If the value hasn't been pulled yet
			...
			// the key "parent", and then use valueForKey( _SOURCE_KEY ) instead of valueForBinding.
