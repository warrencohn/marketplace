...
//  MiniFramework
//
//  Created by Gavin Eadie on Tue Feb 17 2004.
//  Copyright (c) 2004 Ramsay Consulting. All rights reserved.
//
...
*/
    ...
    public CoreApplication () {
        ...
        NSNotificationCenter.defaultCenter().addObserver(this,
                new NSSelector<Object>("applicationDidFinishLaunching", new Class[] { NSNotification.class }),
    ...
    /**
     * Overrides refuseNewSessions(...) so that it only works on weekdays !!
     */
/*
    public void refuseNewSessions(boolean refusing) {
        if (refusing) {
        ...
        super.refuseNewSessions(refusing);
    } 
 */
...
//--------------------------------------------------------------------------- http://patorjk.com/software/taag/ --
     ...
     * This method calls subclasses' {@link #didFinishLaunchingApplication} method.
     *
     * @param n notification that is posted after the WOApplication
	...
	private void adjustEOModel(EOModel model) {
        ...
        logger.info ("--- adjustEOModel - database: " + model.name().toUpperCase() + 
        		     " using connection dictionary from " + ((gotEOModelAdustment) ? "props" : "model"));
        logger.trace ("--- adjustEOModel - [user=" + dict.objectForKey(JDBCAdaptor.UsernameKey) + 
                      "/pass=" + (null == dict.objectForKey(JDBCAdaptor.PasswordKey) ? "NONE" : "****") + 
                      "/URL=" + dict.objectForKey(JDBCAdaptor.URLKey) + "]");
    ...
    private NSMutableDictionary<String, CoreSession>
